...
def index():
    ...
    if posts.has_prev:
        ...
        prev_url=None
    return render_template('index.html', title='Home',next_url=next_url,prev_url=prev_url,form=form, posts=posts.items)
...
def user(email):
    ...
    posts = user.posts.order_by(Post.timestamp.desc()).paginate(
        page, app.config['POSTS_PER_PAGE'], False)
    next_url = url_for('user', email=user.email, page=posts.next_num) \
        if posts.has_next else None
    prev_url = url_for('user', email=user.email, page=posts.prev_num) \
